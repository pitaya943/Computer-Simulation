...
class MM1Queue:
    ...
    def report(self):
        with open('outfile.txt', 'a', encoding='utf-8') as outfile:
            ...
            outfile.write('------------------------------------------\n')
            outfile.write(f'Average delay in queue {(self.total_of_delays / self.num_custs_delayed):15.3f} minutes\n')
            outfile.write(f'Average number in queue {(self.area_num_in_q / self.sim_time):14.3f}\n')
